...
@section('title', 'Tambah API Endpoint')
                ...
                <form action="{{ route('settings.api.store') }}" method="POST">
                    ...
                    <div class="card-body">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="endpoint" id="endpoint" class="form-control @error('endpoint') is-invalid @enderror" 
                                           value="{{ old('endpoint') }}" placeholder="api/dynamic/table_name" required>
                                    @error('endpoint')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                    <small class="text-muted">Contoh: api/dynamic/users, api/dynamic/products/{id}</small>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select name="table_name" id="table_name" class="form-select @error('table_name') is-invalid @enderror" required>
                                        ...
                                        @foreach($dynamicTables as $table)
                                            <option value="{{ $table->table_name }}" {{ old('table_name') == $table->table_name ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            @error('permission_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <small class="text-muted">Jika kosong, API akan menggunakan middleware ApiGuard saja</small>
                        </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-check mb-3">
                                    <input class="form-check-input" type="checkbox" name="use_ip_restriction" value="1" id="use_ip_restriction"
                                           {{ old('use_ip_restriction') ? 'checked' : '' }}>
                        ...
                        <div class="row" id="ip_section" style="{{ old('use_ip_restriction') ? '' : 'display: none;' }}">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <textarea name="ip_whitelist" id="ip_whitelist" class="form-control @error('ip_whitelist') is-invalid @enderror" 
                                              rows="3" placeholder="192.168.1.1&#10;10.0.0.0/8&#10;203.0.113.0/24">{{ old('ip_whitelist') }}</textarea>
                                    @error('ip_whitelist')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <textarea name="ip_blacklist" id="ip_blacklist" class="form-control @error('ip_blacklist') is-invalid @enderror" 
                                              rows="3" placeholder="192.168.1.100&#10;203.0.113.50">{{ old('ip_blacklist') }}</textarea>
                                    @error('ip_blacklist')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                    <small class="text-muted">IP yang diblokir (prioritas lebih tinggi dari whitelist)</small>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-check mb-3">
                            <input class="form-check-input" type="checkbox" name="use_rate_limit" value="1" id="use_rate_limit"
                                   {{ old('use_rate_limit') ? 'checked' : '' }}>
                            <label class="form-check-label" for="use_rate_limit">
                                Gunakan Rate Limiting
                            </label>
                        </div>

                        <div class="row" id="rate_section" style="{{ old('use_rate_limit') ? '' : 'display: none;' }}">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="number" name="rate_limit_max" id="rate_limit_max" 
                                           ...
                                           value="{{ old('rate_limit_max', 60) }}" min="1">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="number" name="rate_limit_period" id="rate_limit_period" 
                                           ...
                                           value="{{ old('rate_limit_period', 60) }}" min="1">
                    ...
                    <div class="card-footer">
                        <button type="submit" class="btn btn-primary">
                            <i class="fas fa-save"></i> Simpan API Endpoint
                        </button>
...
document.addEventListener('DOMContentLoaded', function() {
    // Auto-generate endpoint based on table selection
    document.getElementById('table_name').addEventListener('change', function() {
        const tableName = this.value;
        const method = document.getElementById('method').value;
        
        if (tableName && method) {
            let endpoint = `api/dynamic/${tableName}`;
            if (method === 'GET' && document.getElementById('endpoint').value.includes('{id}')) {
                endpoint += '/{id}';
    ...
    document.getElementById('method').addEventListener('change', function() {
        ...
        if (tableName && method) {
            ...
            if (['PUT', 'DELETE'].includes(method) || 
                (method === 'GET' && document.getElementById('endpoint').value.includes('{id}'))) {
                endpoint += '/{id}';
            }
            document.getElementById('endpoint').value = endpoint;
        }
